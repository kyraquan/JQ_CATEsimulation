...
headers<-data.frame( METHOD = character(),
            level2n=numeric(),level1n=numeric(),ICC=numeric(), ps_model=numeric(),treatment_model=numeric(),outcome_model=numeric(),
            tau_var=numeric(),
...
#ps_model = PS_model,treatment_model = treatment_model,outcome_model = Outcome_model,tau_var=tau_var,
          ...
          for (Outcome_model in c(1,2,3)) { #Outcome model 
            #for (Proportion in c(2.197225,0.8472979)) { #proportion treated, removed, only keeping log(0.3/0.7)
              ...
              for (i in 1:NumIter) { 
                ...
                datatest = PS_model_data(n_cluster=level2n, n_ind=level1n,ICC=ICC,int=(-log(0.3/0.7)),tau_var=tau_var,
                                         ps_model = PS_model,treatment_model = treatment_model,outcome_model = Outcome_model)
                ...
                conditions <- c(level2n,level1n,ICC, PS_model,treatment_model,Outcome_model,tau_var)
