...
assemble_path <- function(job_id, task_id, file_name) {
  path <- file.path(job_id, file_name)
  return(path)
}

#file names 
BARTresults <- paste0(task_id,"_BARTresults.csv")
CFresults <- paste0(task_id,"_CFresults.csv")
SL1results <- paste0(task_id,"_SL1results.csv")
SL2results <- paste0(task_id,"_SL2results.csv")
SL3results <- paste0(task_id,"_SL3results.csv")
TL1results <- paste0(task_id,"_TL1results.csv")
#TL2results <- paste0(task_id,"_TL2results.csv")
TL3results <- paste0(task_id,"_TL3results.csv")
#XL1results <- paste0(task_id,"_XL1results.csv")
#XL2results <- paste0(task_id,"_XL2results.csv")
XL3results <- paste0(task_id,"_XL3results.csv")
BARTXL2results <- paste0(task_id, "_BARTXL2results.csv")
CFXL1results <- paste0(task_id, "_CFXL1results.csv")
BARTTL2results <- paste0(task_id,"_BARTTL2results.csv")
...
tasks <- list(
  ...
  function() {
      ...
      if(class(CFXL1result)[[1]] =="try_error") {
        ...
        evalCFXL1 <- c("XL1",conditions,evalCFXL1)
      }else{
      evalCFXL1 <- Results_evaluation(CFXL1result,datatest)
    evalCFXL1 <- c("XL1",conditions,evalCFXL1) 
    }
  ...
  function() {
    # BARTXL2result <- BART_Xlearner(data.train,data.test,datatest)
    ...
    evalXL2bart <- c("XL2",conditions,evalXL2bart)
    }else{
      evalXL2bart <- Results_evaluation(BARTXL2result,datatest)
      evalXL2bart <- c("XL2",conditions,evalXL2bart)
    }
...
if(class(BARTTL2result)[[1]] =="try_error") {
  ...
  evalTL2bart <- c("TL2",conditions,evalTL2bart)
...
print(paste("Finished iteration ", task_id))
stopCluster(cl)
...
storage_upload(cont, src=BARTTL2results, dest=assemble_path(job_id, task_id, BARTTL2results))
storage_upload(cont, src=TL3results, dest=assemble_path(job_id, task_id, TL3results))
storage_upload(cont, src=CFXL1results, dest=assemble_path(job_id, task_id, CFXL1results))
storage_upload(cont, src=BARTXL2results, dest=assemble_path(job_id, task_id, BARTXL2results))
