...
# xi- covariates; wi-treatment assignment; yi- outcome
...
#' linear regression, intercept and trt assignment
#' Yi = Yi(0) + Wi(Yi(1)-Yi(0))
#' E(Yi|Wi) = a + Wi*tau, where a := E(Yi(0))
...
coef(summary(ols))[2,] #the SE here is not heteroskedasticity-robust 
...
### turning experimental data into observational data 
...
#comparing the original and the dropped 
...
#from this plot, the treated is younger and liberal, while the untreated is older and more conservative.
...
#Augmented inverxe propensity-weighted (AIPW)
# Available in randomized settings and observational settings with unconfoundedness+overlap
...
print(forest.ate)
#' Finally, even when overlap holds, when the propensity score is small AIPW 
#' can have unstable performance. In that case, 
#' other estimators of the average treatment effect are available. 
#' For example, see the approximate residual balancing (ARB) method 
#' in Athey, Imbens and Wager (2016) and its accompanying R package balanceHD.
...
# change this if working on different data set; covariate histogram on the unadjusted
...
#covariate histogram on the adjusted
